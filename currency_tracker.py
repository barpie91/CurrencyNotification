 ...
 compare rates with the given thresholds, set and check alerts, and send notification to the destination user.
  ...
  def comparison(self, currency_code, treshold_low, treshold_high):
      """It checks if there are any items above/below given threshold.
	      ...
	      Returns: Robi detekcje anomalii w oparciu o górny i dolny threshold. Zwraca liste dat kiedy waluta przekroczyla threshold dolny i górny
      ...
      is_greater_than_treshold = self.df[self.currency_value_column].gt(treshold_high).any()
      if is_greater_than_treshold:
        above_treshold = self.df[self.df[self.currency_value_column] > treshold_high]
        above_treshold_list = above_treshold[self.data_column].tolist()
      else:
        above_treshold_list = []

      is_lower_than_treshold = self.df[self.currency_value_column].lt(treshold_low).any()

      if is_lower_than_treshold:
        below_treshold = self.df[self.df[self.currency_value_column] < treshold_low]
